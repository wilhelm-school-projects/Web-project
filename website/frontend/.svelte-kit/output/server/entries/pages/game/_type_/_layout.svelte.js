...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let texts = ["Draw", "Clear", "dummy"];
  return `


<nav id="bottom-navbar" class="navbar row padding-bottom justify-content-center d-flex">${each(texts, (text, i) => {
    return `<button id="${"button-" + escape(text, true) + "-drawing-pane"}" class="col text-dark text-center btn btn-outline-secondary">${escape(texts[i])}
        </button>`;
  })}</nav>
